...
export function handleConfigure(config: InternalAxiosRequestConfig<any>) {
  const { apiUrl, token } = useProfileStore()
  config.baseURL = apiUrl
  config.headers['X-Auth-Token'] = token
  return config
}

export function handleNetworkError(errStatus?: number): string {
